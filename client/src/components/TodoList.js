...
const TodoList = () => {
	...
	const fetchTodos = async () => {
		try {
			...
			setNoDataFound(fetchedTodos.length === 0); // Check if no data found
		} catch (error) {
	...
	return (
		<div className="mx-auto container max-w-4xl text-center my-14 bg-slate-200 rounded-xl flex flex-col items-center">
			...
			<Button
				...
				outline
				pill
				className="w-40 mt-5 bg-purple-600"
			...
			{isLoading ? (
				...
				<div className="w-full flex flex-col items-center">{renderTodos()}</div>
			)}
